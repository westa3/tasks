...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    const trimmedAnswer = lowerAnswer.trim();
    const expectedValue = question.expected.toLowerCase();
...
export function toShortForm(question: Question): string {
    ...
    const QuesName = question.name.slice(0, 10);
    return QuesID + ": " + QuesName;
}
...
export function toMarkdown(question: Question): string {
    const sep = "\n" + "- ";
    const strOpt = question.options.join(sep);
    let str = "";
...
export function publishQuestion(question: Question): Question {
    const NewPub = !question.published;
    return { ...question, published: NewPub };
}
